...
#-----------------------------------------------------
#PATHS:
multilevelhoppath= "/home/bastian/bachelorarbeit_Projekte/Projekte/pca_mos2/ressources/multilevelhops.txt"
...
#multilevelhoppingslist: based on idealhoppingslist, but ordered by order of neighbourhood.
#list is built with multilevelhoplistbuilder.py, which orders the idealhoppingslist file.
multilevelhopfile = open(multilevelhoppath,"r")
content = multilevelhopfile.read()
multilevelhoppingslist = ast.literal_eval(content)

#allhoppingslist takes all the hoppings in one list, independent of their order
allhoppingslist = []
for hopvec in multilevelhoppingslist.copy():
    for hop in hopvec:
        allhoppingslist.append(hop)
#-----------------------------------------------------
#CLASSES:

#mcell_multilevel: represents a primitive cell with hoppings
class mcell_multilevel:
    ...
    def __init__(self,name,E_min):
        ...
        self.mhoppings = allhoppingslist.copy()

        self.mhoppings_ml = multilevelhoppingslist.copy()
        self.mnhoppings_ml = [len(hoplist) for hoplist in self.mhoppings_ml]
    ...
    def mchangehops_tohopvec_allorders(self,newhoppingvector_ml):
        self.mclearhoppings()
