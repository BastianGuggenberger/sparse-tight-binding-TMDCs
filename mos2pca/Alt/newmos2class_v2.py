...
dset = [0.0, 0.24, 0.32] #Distances of different orders
...
class mcell:
    ...
    #-----------------------------------------------------
    #METHODS of class mcell, specific for multilevel implementation:

    #returns the hoppingvector of hoppings of a certain degree of order
    def mgetmhoppings_ml(self):
        allhops = self.mhoppings.copy()
        ml_hopvec = [[],[],[]]
        ...
        return ml_hopvec.copy()
    
    #deletes all hopping of a certain degree of order
    def mclearhoppings_sl(self,order):
        hopvec_ml = self.mgetmhoppings_ml()
        hopvec_sl = hopvec_ml[order] #this creates a link, not a copy!!
        length = len(hopvec_sl)
        for i in range(length):
            self.mprimcell.remove_hopping(hopvec_sl[0][0],hopvec_sl[0][1],hopvec_sl[0][2])
            del hopvec_sl[0]
