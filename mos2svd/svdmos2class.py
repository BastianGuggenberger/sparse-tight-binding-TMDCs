...
#Ideal Hopping List: (All Hoppings)
idealhoppingslist = []
idealhopfile = open("idealhops.txt","r")
...
class msvdcell:
    ...
    def mtruncate(self,n_components):
        #1. Get T - Matrix
        ...
        for hop in hopvec:
            if(hop[0] in hopdict.keys()):
                ...
                hopdict[hop[0]] = np.zeros((self.norb,self.norb))
                hopdict[hop[0]][hop[1],hop[2]] = hop[3]
                hopdict[hop[0]][hop[2],hop[1]] = hop[3].conj()
        ...
        for r_index,R in enumerate(r_vec):
            hamiltonian = T[r_index].reshape(self.norb,self.norb)
            ...
            for i in range(self.norb):
                for j in range(i+1):
                    ...
                    if(abs(Ehop)>self.E_min):
                        newhopvec.append([R,i,j,Ehop])   
        
        self.changehops_tohopvec(newhopvec)  
