...
#Handles the Bands near the Bandgap with more weight.
...
bandgapweight = 3.0
lambda_1vec = [8.0]
runvec = [60000] #Increase by 1 for every run !!!! #Declared
...
def EF(x,currentcell):
    ...
    metric = mmetric(currentcell,ideal_bands_efficient,efficient=True, weight_bandgap= bandgapweight)
...
def nesterovgd(run,lambda_1,printinfos=True):
    ...
    paramsfile.writelines("bandgwapweight = " + str(bandgapweight) + "\n")
    ...
    for wdh in range(iterations):
        ...
        string += "metric=" + str(mmetric(resultcell,ideal_bands, weight_bandgap=bandgapweight)) + "\n" + "\n" #Here the less efficient, more accurate metric is calculated
