...
def mmetric(cell, comparison_bands, efficient = False, weight_bandgap = 1):
    ...
    for i in range(numbands_vector[0]):
        for j in range(bands_vector[0].shape[0]):
            ...
            currenterror = (E_cell-E_comparison)**2
            if(weight_bandgap != 1):
                if((E_cell <= 2.0 and E_cell >= -2.0 ) or (E_comparison <= 2.0 and E_comparison >= -2.0)):
                    currenterror = weight_bandgap * currenterror 
            if(currenterror >= 0.1):
                error += currenterror
